...
ref_index=np.loadtxt("kiana.csv")
lam = ref_index[:,0]
...
ax.set(xlabel='Wavelength [nm]', ylabel='Efficiency [a.u.]', title='{}'.format(diameter)+' nm Aunanosphere')
ax.legend(['Absorption', 'Scattering', 'Extinction'])
plt.show()
# Spectrum to color conversion
# Load CIE 1931 2-deg color matching functionshttp://.ioo.ucl.ac.uk/ CIE
CMF=np.loadtxt("ciexyz31_1.xls")
x = CMF[:,1]
y = CMF[:,2]
z = CMF[:,3]
# Load relative spectral power of a standard illuminant D65
D65 = CMF[:,4]
# Load conversion matrix from XYZ coordinates to sRGBcvrl
...
rgb = np.zeros((3,3))
for i in range(3):spectrum = spectra[:,i]
